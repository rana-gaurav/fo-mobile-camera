package com.faceopen.camerabenchmark.dialog;
...
public class LabeledSwitch extends ToggleableView {
    ...
    private void initView() {
        this.isOn = false;
        this.labelOn = "Save";
        this.labelOff = "Delete";

        this.enabled = true;
        this.textSize = (int) (12f * getResources().getDisplayMetrics().scaledDensity);

        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            colorBorder = colorOn = getResources().getColor(R.color.purple_700, getContext().getTheme());
        } else {
            colorBorder = colorOn = getResources().getColor(R.color.purple_700);
        }
    ...
    private void initProperties(AttributeSet attrs) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            if (attr == R.styleable.Toggle_on) {
                ...
                int accentColor;
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    accentColor = getResources().getColor(R.color.purple_700, getContext().getTheme());
                } else {
                    accentColor = getResources().getColor(R.color.purple_700);
                }
